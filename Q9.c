            ...
            if (commande->seq == NULL) {                /* Ligne vide */
                ...
                if (pidFils == -1) {
                    ...
                    if (commande->seq[1] != NULL) {
                        ...
                        if (pid1 == -1) {
                        ...
                        } else if (pid1 == 0) {             /* fils 0 */
                            ident_fils = 0;

                            close(p[0]);
                            if (dup2(p[1], STDOUT_FILENO) == -1) {
                                fprintf(stderr, "dup2 failed - tubue fils 0");
                            }                            
                            ...
                            if (dup2(p[0], STDIN_FILENO) == -1) {
                                fprintf(stderr, "dup2 failed - tube fils 1");
                            }
                            ajoutProcessus(pid1, commande);    // On attend que la première commande s'execute
                            waitpid(pid1, NULL, WUNTRACED);
                            retirerProcessus(pid1);
                        }
                        execvp(commande->seq[ident_fils][0], commande->seq[ident_fils]);
                        exit(pid1);  //Si erreur execvp
